...
MaptrackerLogs(mode := "")
{
	...
	hFont := settings.maptracker.fHeight * 1.5, entries := vars.maptracker.entries := {}, max_lines := Floor(vars.monitor.h*0.8 / hFont)
	...
	If entries.Count()
	{
		...
		If !vars.maptracker.active_date ;&& !Blank(LLK_HasVal(ddl, vars.maptracker.active_date, 1)))
			active_date := vars.maptracker.active_date := SubStr(ddl[ddl.Count()], 1, InStr(ddl[ddl.Count()], " ") - 1)
		Else If (mode = "refresh")
			Return
	...
	Gui, %GUI_name%: Add, Text, % "ys HWNDhwnd00 x+" settings.maptracker.fWidth . (!vars.maptracker.keywords.Count() ? "" : " cLime"), % LangTrans("maptracker_search")
	...
	For index, val in table
	{
		...
		width := (width < hFont) ? hFont : width * (header = "content" ? 3 : 1), header_tooltips := ["map", "e-exp", "deaths", "kills", "loot", "mapinfo", "notes", "content", "tier", "run"]
		If (header = "#")
		{
			...
			If (active_date = "all") || InStr(date, vars.maptracker.active_date) || date_isLeague && LLK_IsBetween(StrReplace(date, "/"), vars.maptracker.leagues[date_isLeague].2, vars.maptracker.leagues[date_isLeague].3)
				For index, content in array
				{
					...
					If (header = "content") && content[header]
						Loop, Parse, % content[header], `;, %A_Space%
						{
							Gui, %GUI_name%: Add, Pic, % (A_Index = 1 ? "xp+"settings.maptracker.fWidth/4 + 1 " yp+" 1 + settings.maptracker.fHeight * 0.1 " h" settings.maptracker.fHeight * 1.3 : "x+"settings.maptracker.fWidth/4 " yp hp") " w-1 HWNDicon gMaptrackerLogsFilter BackgroundTrans", % "img\GUI\mapping tracker\" A_LoopField ".png"
